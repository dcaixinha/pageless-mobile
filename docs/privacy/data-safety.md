...
| ID | Severity | Finding |
| --- | --- | --- |
| ~~**F1**~~ | **Fixed** | `android:allowBackup` was `"true"` with no backup rules, making the bearer token and email in the session DataStore, plus the whole Room DB, eligible for Android Auto Backup to Google's cloud and for device-to-device transfer. Now `"false"` (`AndroidManifest.xml`), with the reasoning recorded in the manifest and in `AGENTS.md`. Verified in both merged debug and release manifests. Closed as `pm-a6l.17`. |
...
| **F4** | Medium | Release `network_security_config.xml` trusts `<certificates src="system" />` only, while its own comment claims user-added CAs are honoured. Self-hosters using a private CA will fail to connect on release builds. Not a Data safety issue, but it constrains **pm-a6l.2**: the review server needs a publicly trusted certificate. |
